...
object TypeCoercion {
  ...
  val findTightestCommonType: (DataType, DataType) => Option[DataType] = {
    ...
    case (t1@StructType(fields1), t2@StructType(fields2)) if t1.sameType(t2) =>
      Some(StructType(fields1.zip(fields2).map { case (f1, f2) =>
        ...
        // except `name` (in case of `spark.org.apache.spark.sql.caseSensitive=false`) and `nullable`.
  ...
  /**
    ...
    *
    * If `spark.org.apache.spark.sql.function.concatBinaryAsString` is false and all children types are binary,
  ...
  /**
    ...
    *
    * If `spark.org.apache.spark.sql.function.eltOutputAsString` is false and all children types are binary,
    * the expected types are binary. Otherwise, the expected ones are strings.
    */

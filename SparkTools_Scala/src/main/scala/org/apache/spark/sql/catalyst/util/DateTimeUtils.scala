 ...
 * Dates are exposed externally as java.org.apache.spark.sql.Date and are represented internally as the number of
 * dates since the Unix epoch (1970-01-01). Timestamps are exposed externally as java.org.apache.spark.sql.Timestamp
...
object DateTimeUtils {
  ...
  /**
   * Returns the number of days since epoch from java.org.apache.spark.sql.Date.
   */
  ...
  /**
   * Returns a java.org.apache.spark.sql.Date from number of days since epoch.
   */
  ...
  /**
   * Returns a java.org.apache.spark.sql.Timestamp from number of micros since epoch.
   */
  ...
  /**
   * Returns the number of micros since epoch from java.org.apache.spark.sql.Timestamp.
   */
  ...
  private[sql] def getOffsetFromLocalMillis(millisLocal: Long, tz: TimeZone): Long = {
    ...
    if (offset != guess) {
      ...
      if (guess != offset) {
        // fallback to do the reverse lookup using java.org.apache.spark.sql.Timestamp

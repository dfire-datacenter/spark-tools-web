...
case class Cast(child: Expression, dataType: DataType, timeZoneId: Option[String] = None)
    ...
    from match {
      ...
      case DateType =>
        (c, evPrim, evNull) => s"""$evPrim = UTF8String.fromString(
          org.apache.spark.org.apache.spark.sql.catalyst.util.DateTimeUtils.dateToString($c));"""
      case TimestampType =>
        val tz = ctx.addReferenceObj("timeZone", timeZone)
        (c, evPrim, evNull) => s"""$evPrim = UTF8String.fromString(
          org.apache.spark.org.apache.spark.sql.catalyst.util.DateTimeUtils.timestampToString($c, $tz));"""
      case ArrayType(et, _) =>
        (c, evPrim, evNull) => {
  ...
  private[this] def castToDateCode(
    ...
    case StringType =>
      ...
      (c, evPrim, evNull) => s"""
        scala.Option<Integer> $intOpt =
          org.apache.spark.org.apache.spark.sql.catalyst.util.DateTimeUtils.stringToDate($c);
        if ($intOpt.isDefined()) {
    ...
    case TimestampType =>
      val tz = ctx.addReferenceObj("timeZone", timeZone)
      (c, evPrim, evNull) =>
        s"$evPrim = org.apache.spark.org.apache.spark.sql.catalyst.util.DateTimeUtils.millisToDays($c / 1000L, $tz);"
    case _ =>
  ...
  private[this] def castToTimestampCode(
      ...
      (c, evPrim, evNull) =>
        s"""
          scala.Option<Long> $longOpt =
            org.apache.spark.org.apache.spark.sql.catalyst.util.DateTimeUtils.stringToTimestamp($c, $tz);
    ...
    case DateType =>
      val tz = ctx.addReferenceObj("timeZone", timeZone)
      (c, evPrim, evNull) =>
        s"$evPrim = org.apache.spark.org.apache.spark.sql.catalyst.util.DateTimeUtils.daysToMillis($c, $tz) * 1000;"
    case DecimalType() =>

 ...
 * see the static org.apache.spark.sql configs via `SparkSession.conf`, but can NOT set/unset them.
 */
object StaticSQLConf {

  import SQLConf.buildStaticConf

  val WAREHOUSE_PATH = buildStaticConf("spark.org.apache.spark.sql.warehouse.dir")
    .doc("The default location for managed databases and tables.")
    .stringConf
    .createWithDefault(Utils.resolveURI("spark-warehouse").toString)

  val CATALOG_IMPLEMENTATION = buildStaticConf("spark.org.apache.spark.sql.catalogImplementation")
    .internal()
    .stringConf
    .checkValues(Set("hive", "in-memory"))
    .createWithDefault("in-memory")

  val GLOBAL_TEMP_DATABASE = buildStaticConf("spark.org.apache.spark.sql.globalTempDatabase")
    .internal()
    .stringConf
  ...
  val SCHEMA_STRING_LENGTH_THRESHOLD =
    buildStaticConf("spark.org.apache.spark.sql.sources.schemaStringLengthThreshold")
  ...
  val FILESOURCE_TABLE_RELATION_CACHE_SIZE =
    buildStaticConf("spark.org.apache.spark.sql.filesourceTableRelationCacheSize")
      .internal()
  ...
  val DEBUG_MODE = buildStaticConf("spark.org.apache.spark.sql.debug")
    .internal()
  ...
  val HIVE_THRIFT_SERVER_SINGLESESSION =
    buildStaticConf("spark.org.apache.spark.sql.hive.thriftServer.singleSession")
  ...
  val SPARK_SESSION_EXTENSIONS = buildStaticConf("spark.org.apache.spark.sql.extensions")
  ...
  val QUERY_EXECUTION_LISTENERS = buildStaticConf("spark.org.apache.spark.sql.queryExecutionListeners")
  ...
  val UI_RETAINED_EXECUTIONS =
    buildStaticConf("spark.org.apache.spark.sql.ui.retainedExecutions")

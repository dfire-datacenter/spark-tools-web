...
case class Uuid(randomSeed: Option[Long] = None) extends LeafExpression with Nondeterministic {
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    ctx.addMutableState("org.apache.spark.org.apache.spark.sql.catalyst.util.RandomUUIDGenerator", randomGen,
      forceInline = true,
      useFreshName = false)
    ctx.addPartitionInitializationStatement(s"$randomGen = " +
      "new org.apache.spark.org.apache.spark.sql.catalyst.util.RandomUUIDGenerator(" +
